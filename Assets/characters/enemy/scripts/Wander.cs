...
public class Wander : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("Player") && followPlayer)
        {
            ...
            if (GetComponent<projectileAttack>() != null && shootCoroutine == null)
            {

                shootCoroutine = StartCoroutine(shooting(collision.gameObject.transform));
    ...
    private IEnumerator shooting(Transform target) {
        ...
        while (true)
        {
            ...
            projectiles attack = gameObject.GetComponent<projectileAttack>().obj;

            attack.angle = projectiles.setAngle(gameObject.transform.position, target.position);

            Instantiate(attack, new Vector3(gameObject.transform.position.x, gameObject.transform.position.y, gameObject.transform.position.z), Quaternion.Euler(0,0, attack.angle));
